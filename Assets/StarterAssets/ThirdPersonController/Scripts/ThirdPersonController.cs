...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            _hasAnimator = TryGetComponent(out _animator); // ← only in Start
        ...
        private void Move()
        {
            float targetSpeed = _input.move.magnitude > 0.5f ? SprintSpeed : MoveSpeed;

            if (_input.move == Vector2.zero) targetSpeed = 0.0f;

            // ← fix: set strafe speed BEFORE animationBlend
            if (strafe) targetSpeed = StrafeSpeed;
            ...
            // animationBlend now uses correct targetSpeed including strafe
            ...
            if (strafe)
            {
                // strafe speed already set above ← removed duplicate line
                ...
                if (_hasAnimator)
                {
                    ...
                    _animator.SetFloat("X", Mathf.Lerp(currentX, inputDirection.x,
                        SpeedChangeRate * Time.deltaTime));
                    _animator.SetFloat("Y", Mathf.Lerp(currentY, inputDirection.z,
                        SpeedChangeRate * Time.deltaTime));
                }
            }
            else
